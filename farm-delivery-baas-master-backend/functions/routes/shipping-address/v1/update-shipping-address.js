...
router.put('/', authenticateMiddleware, (req, res) => {
    ...
    getCustomer(req.user.email).then((users) => {
        if(users.length){
            ...
            //Construct URL
            let updateShippingAddressUrl = 'https://' + settings.bigcommerce_api_host +':443'
            + '/stores/' + settings.bigcommerce_store_hash 
            + '/v3/customers/addresses';

            axios.put(updateShippingAddressUrl, payload, config)
                .then((response) => {
